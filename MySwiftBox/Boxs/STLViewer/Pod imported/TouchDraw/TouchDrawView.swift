...
@objc public protocol TouchDrawViewDelegate {
    ...
    /// CUSTOM ADD BY ME triggered when endDrawing
    @objc optional func endDrawing()
    
    /// CUSTOM ADD BY ME triggered when startDrawing
    @objc optional func detectDrawingElement(_ touch: UITouch)
}
...
open class TouchDrawView: UIView {
    ...
    open func updateSize(_ size : CGSize) {
        
        let diffWidth = size.width/self.imageView.frame.width
        let diffHeight = size.height/self.imageView.frame.height
        
        stack.forEach { (stroke) in
            stroke.points = stroke.points.map({ (prev) -> CGPoint in
                let x = prev.x * diffHeight
                let y = prev.y * diffWidth
    ...
    override open func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let touch = touches.first {
            if isDrawing {
                ...
                delegate?.detectDrawingElement?(touch)
            }
        }
    }
